...
def normalize(records: list[dict], *, text_start: int | None = None, text_size: int | None = None) -> dict:
    ...
    for left, right in zip(normalized, normalized[1:]):
        if right["address"] < left["end_address"]:
            overlaps.append({
                "left": left["symbol"],
                "right": right["symbol"],
                "range": [right["address"], min(left["end_address"], right["end_address"])],
            })
    ...
    result = {
        ...
        "overlaps": overlaps,
        "notes": [
            "Default sub_XXXXXXXX names are neutral provisional identifiers, not semantic claims.",
            "Semantic renames require evidence and should retain provenance in the evidence field.",
        ],
    }
