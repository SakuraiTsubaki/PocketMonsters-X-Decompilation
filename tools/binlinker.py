...
def parse_binlinker_header(data: bytes, *, byteorder: str) -> BinLinkerHeader:
    """Parse only the documented BinLinker header and raw offset table."""
...
def candidate_byteorders(data: bytes) -> tuple[str, ...]:
    """Return byte orders whose declared offset table fits in the input.

    This is only a structural hint. A surviving candidate is not proof of the
    real byte order, because payload semantics are deliberately not guessed.
    """
    candidates: list[str] = []
...
def main() -> int:
    ...
    parser.add_argument(
        "--byteorder",
        choices=("little", "big"),
        help="explicit byte order; if omitted, report structural candidates",
    )
