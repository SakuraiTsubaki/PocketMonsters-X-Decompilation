...
class FunctionManifestTests(unittest.TestCase):
    def test_provisional_names_and_sorting(self) -> None:
        result = normalize([
            {"address": "0x1010", "size": 4},
            {"address": 0x1000, "size": 8, "symbol": "KnownByEvidence", "status": "observed", "evidence": ["xref:test"]},
        ])
    ...
    def test_overlap_and_coverage(self) -> None:
        result = normalize([
            ...
            {"address": 0x1008, "size": 0x10},
            {"address": 0x1020, "size": 0x10},
        ], text_start=0x1000, text_size=0x40)
