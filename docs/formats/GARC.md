...
Status: **public-source reconstruction; retail Pokémon X dump not yet available for direct verification**
...
Without a Pokémon X source dump, the following remain unverified here:

- exact raw magic bytes in a retail X archive
- whether all retail X GARCs use identical padding behavior
...
7. When a lawful Pokémon X dump becomes available, verify parse → repack behavior and record hashes/revision metadata without committing the ROM itself.
...
- Pokémon X path inventory: **blocked pending source dump**
