...
The comparison origin for this repository is the **Japan-market release of Pokémon X**. This is a research baseline, not a claim that the Japanese retail build is the development master for every worldwide release.
...
Generation VI also separates **market/region** from **in-game language**. The official Japanese Pokémon X/Y site states that a new save can select Japanese, English, Spanish, French, German, Italian, or Korean, and that the selected language cannot be changed during that save. Consequently, `Japan` must not be treated as synonymous with `Japanese-language data`.
