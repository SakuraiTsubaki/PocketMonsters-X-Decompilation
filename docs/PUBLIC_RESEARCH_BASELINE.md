...
No local Pokémon X ROM, cartridge dump, CIA/CCI, extracted RomFS, or extracted ExeFS is currently available to this project.

Accordingly, this repository must distinguish **publicly documented/reconstructed knowledge** from **facts verified directly against a Pokémon X dump**. Until a lawful source dump is available, exact hashes, offsets, archive contents, per-revision binary layouts, and rebuild equivalence remain unverified.
...
- Pokémon X documentation is directly relevant, but every statement should still be independently verified when source files become available.
...
- Pokémon X file-by-file RomFS/ExeFS inventory verified from the retail title.
